...
# The reasoning method reaches the session whole: inside the runtime's preview
...
out=$(CLAUDE_PLUGIN_ROOT="$root" bash "$root/hooks/session-reasoning.sh" <&-)
...
[[ "$(head -1 <<<"$out")" == "# How you think" ]] || bad "the block does not open by saying what it is"
...
for s in mellions-deep-research mellions-falsification mellions-self-learning; do
...
mkdir -p "$tmp/hooks" && cp "$root/hooks/session-reasoning.sh" "$tmp/hooks/"
missing=$(CLAUDE_PLUGIN_ROOT="$tmp" bash "$tmp/hooks/session-reasoning.sh" <&-)
grep -q 'REASONING METHOD NOT FOUND' <<<"$missing" || bad "a missing Skill produces no message, so a partial install is silent"

[[ $fail -eq 0 ]] && echo "ok  session-reasoning"
exit $fail
