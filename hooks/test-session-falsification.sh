...
# The falsification method reaches the session whole: inside the runtime's preview
...
out=$(CLAUDE_PLUGIN_ROOT="$root" bash "$root/hooks/session-falsification.sh" <&-)
...
[[ "$(head -1 <<<"$out")" == "# How you prove a fix holds" ]] || bad "the block does not open by saying what it is"
...
for s in mellions-deep-research mellions-reasoning; do
...
mkdir -p "$tmp/hooks" && cp "$root/hooks/session-falsification.sh" "$tmp/hooks/"
missing=$(CLAUDE_PLUGIN_ROOT="$tmp" bash "$tmp/hooks/session-falsification.sh" <&-)
grep -q 'FALSIFICATION METHOD NOT FOUND' <<<"$missing" || bad "a missing Skill produces no message, so a partial install is silent"

[[ $fail -eq 0 ]] && echo "ok  session-falsification"
exit $fail
