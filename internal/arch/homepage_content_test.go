...
func TestHomepageContentIsStrictBilingualProjectSource(t *testing.T) {
	...
	for localeName, localeRaw := range document.Locales {
		...
		if len(locale.Evidence.Links) != 2 {
			t.Errorf("%s evidence links = %d, want 2", localeName, len(locale.Evidence.Links))
		}
		if len(locale.Support.Items) != 2 {
			t.Errorf("%s support items = %d, want 2", localeName, len(locale.Support.Items))
		}
